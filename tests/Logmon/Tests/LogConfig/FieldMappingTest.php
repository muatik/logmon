...
class FieldMapperTest extends \PHPUnit_Framework_TestCase
{
	...
	public function providerFieldMapping()
	{
		$fm1 = array(
			'data' => array(
				...
				'mapped' => array('unique' => 4, 'date'=> '2013-12-15 16:12:10', 'level' => 'warning', 'message' => 'This is a nice log entry.')
			),
			'mapping' => (object) array(
				'unique' => (object) array('fieldName'=>'id', 'regex' => '(.*)'),
				'level' => (object) array('fieldName'=>'debugCtg', 'regex' => '(.*)'),
		...
		$fm2 = array(
			...
			'mapping' => (object) array(
				...
				'level' => (object) array('fieldName'=>'line', 'regex' => '^\d+ \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} ([a-z]+)'),
	...
	public function testGetFieldMapping($fm)
	{
		...
		$mapped = json_decode($this->mapper->toJson(true));
